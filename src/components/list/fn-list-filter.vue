<template>
   <div class="layer_popup" :class="{layer_z:!show}">
        ...
        <div class="outbox outbox2" :class="{leavebox:!show,toggle:show,lf_zero:selectAdrList}">
            <!-- 筛选第一层 S -->
            <div class="filter_box" :class="{lf:selectAdrList}" v-show="show1">
                        ...
                        <ul class="filter_ul filter_ul_btn" id="filter_ul">
                            <li class="sidebar-content">
                                <a href="javascript:void(0);">
                                    <i class="arrow" data-icon2="&#xe602;" data-icon="&#xe60b;"></i>
                                    <p class="sort-of-brand">配送至</p>
                                    <p class="main_color" id="J_delivery_txt"
                                       v-if="filter_address && filter_address.area"
                            ...
                            <li v-if="globalData" class="sidebar-item globalFilter clearfix" data-icon2="&#xe602;"
                                ...
                                <p v-for="child in filter_1Children" :data-key="child.key" :data-name="child.name"
                                   ...
                                   @click="globalBtn(child,$index+1)">
                                    {{child.name}}</p>
                            </li>

                            <li class="sidebar-item price_area">
                                <div class="pe_qj"><p>价格区间:</p>
                                    <input @focus="focusPrice" type="number" class="fnwap_input" placeholder="最低价"
                                           v-model="minPrice">
                                    <p>—</p>
                                    <input @focus="focusPrice" type="number" class="fnwap_input" placeholder="最高价"
                                           v-model="maxPrice">
                                </div>
                            </li>
                            <li class="sidebar-item cate-item marginb" v-if="categoryShow" data-icon2="&#xe602;"
                                data-icon="&#xe60b;"
                                @click="listBtn">
                                <p>分类</p>
                            ...
                            <template v-for="(children, $index) in filter_2Children">
                                <li class="sidebar-item fold filter_attr" :class="{show:children.active}"
                                    data-icon3="&#xe623;"
                                    data-icon="&#xe60b;"
                                    @click="upDown($index)">
                                    <p>{{children.at_name}}</p>
                                    <div v-if="!getSelectedTypeByIndex($index)">全部</div>
                                    <div v-if="getSelectedTypeByIndex($index)" class="J_filter_all main_color"
                                         :data-index="$index">
                ...
                <div class="three_level" :class="{hide_it:listShow}">
                    <div class="filter_title">
                        ...
                        <h3>分类</h3>
                        <div class="sure" @click="cateConfirm">确定</div>
                    </div>
                   <!--  <div class="t_content">
                        <dl class="Bbottom"
                            v-for="children in filter_ClassifiyChildren"
                            :class="{fold:!children.isOpen && $index!==0,unfold:children.isOpen}">
                            <dt class="s" :class="{cur:$index==0 && children.allCate}"
                                @click="listClassBtn(children,children.allCate,$index)">
                                {{children.cp_name}}
                            </dt>
            ...
            <!--</div>-->
            <!-- 筛选第二层 - 地址分类E -->
            <fn-address v-show="showAddress"
                        :address-name="addressName" :sel-show="selShow" :is-cache="isCache"
                        :top-cache="topCache"></fn-address>
        </div>
...
<script>
  ...
  export default {
    props: ['bodyFilters', 'categoryList', 'show1', 'freshShowCate'],
    data(){
      return {
        minPrice: '',
        maxPrice: '',
    ...
    methods: {
      ...
      //点击 全球标
      globalBtn(child,index){
        this.toggleState({target: child, key: 'isSelected'});
        let buttonName = {
          "button_name": child.name,
          "status": child.isSelected ? 1 : 0
        }
        let data = {
          track_tag_name: '2015',
          message: index,
          remarks: JSON.stringify(buttonName),
          track_type: '2'
        };
        $.stat.shangbao(data);
      },
...
<style>
  ...
  .fn_list_container .lf {
    left: 20%;
  }

  .fn_list_container .lf_zero {
    left: 0;
  }
